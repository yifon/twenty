            ...
            if( !respond ){
              ...
              if( respond.file ){
                ...
                return req.bus.fire('respond.data.before', respond).then(function() {
                  logger.log("respond","data ======>".green, _.isObject(respond.data) ?  Object.keys(respond.data).join(",") : respond.data)
                  if( _.isObject(respond.data) ){
                    res.json(respond.data || {})
                  }else{
                    res.send(respond.data.toString())
                  }
                })
              }
            }
